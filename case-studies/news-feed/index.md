...
| --- | --- |
| What is it? | News Feed System is a system design problem topic used to make a specific engineering decision explicit. |
...
News Feed System exists to test whether you can turn ambiguous product behavior into requirements, APIs, state, capacity, bottlenecks, and tradeoffs.
...
flowchart LR
    ...
    Edge --> API["API layer"]
    API --> Core["Core News Feed System service"]
    Core --> Cache["Cache"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For News Feed System, discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect News Feed System to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does News Feed System solve better than the simpler alternative?
...
5. What would make you reject News Feed System?
...
- News Feed System is useful only when its core tradeoff matches the prompt.
...
1. Define News Feed System in one sentence.
...
Use News Feed System when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid News Feed System when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce News Feed System behind a narrow interface or compatibility layer.
...
News Feed System changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- News Feed System solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use News Feed System when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of News Feed System? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
