...
| --- | --- |
| Core idea | Backend for Frontend (BFF) helps candidates discuss architectural trade-offs, scalability, reliability, maintainability, and organizational impact with clarity. |
...
- Define Backend for Frontend (BFF) in one or two crisp sentences before expanding.
...
~~~mermaid
flowchart LR
    A["Clarify the prompt"] --> B["Define Backend for Frontend (BFF)"]
...
~~~mermaid
flowchart TD
    Problem["Problem or requirement"] --> Concept["Backend for Frontend (BFF)"]
...
| --- | --- |
| Problem first | Explain what problem Backend for Frontend (BFF) solves before naming tools or patterns. |
...
| Scenario | How Backend for Frontend (BFF) Shows Up |
| --- | --- |
...
| Front | Back |
| --- | --- |
| What problem does Backend for Frontend (BFF) solve? | It helps structure a decision around requirements, constraints, trade-offs, and production impact. |
...
1. Explain Backend for Frontend (BFF) to an interviewer in 90 seconds.
