...
These notes were moved from the old `50-system-design-patterns/circuit-breaker` bucket during the taxonomy cleanup.
...
| --- | --- |
| What is it? | Circuit Breaker and Bulkhead is a design pattern topic used to make a specific engineering decision explicit. |
...
Circuit Breaker and Bulkhead exists because real systems need a reusable way to manage load, coupling, correctness, latency, or change.
...
```mermaid
flowchart LR
    Smell["Design pressure"] --> Pattern["Apply Circuit Breaker and Bulkhead"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Circuit Breaker and Bulkhead, discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Circuit Breaker and Bulkhead to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Circuit Breaker and Bulkhead solve better than the simpler alternative?
...
5. What would make you reject Circuit Breaker and Bulkhead?
...
- Circuit Breaker and Bulkhead is useful only when its core tradeoff matches the prompt.
...
1. Define Circuit Breaker and Bulkhead in one sentence.
...
Use Circuit Breaker and Bulkhead when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Circuit Breaker and Bulkhead when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Circuit Breaker and Bulkhead behind a narrow interface or compatibility layer.
...
Circuit Breaker and Bulkhead changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Circuit Breaker and Bulkhead solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Circuit Breaker and Bulkhead when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Circuit Breaker and Bulkhead? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
