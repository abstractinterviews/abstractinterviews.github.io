...
subtitle: "Request Response for senior engineering and architecture interviews."
...
Request Response is an architecture topic used to reason about system structure, ownership, communication, data flow, and operational behavior. In interviews, do not stop at the definition. Explain the constraints that make it attractive and the trade-offs it introduces.
...
1. Define Request Response in one sentence.
...
Use Request Response when its benefits directly match the system constraints. Avoid it when it adds coordination, deployment, data consistency, or operational overhead without a clear payoff.



## Imported System Design Notes

These notes were moved from the old `50-system-design-patterns/request-response` bucket during the taxonomy cleanup.
...
| --- | --- |
| What is it? | Request Response is a architecture pattern topic used to make a specific engineering decision explicit. |
...
Request Response exists to help teams make structural decisions that remain understandable as the system, organization, and traffic grow.
...
```mermaid
flowchart LR
    Drivers["Business and quality drivers"] --> Decision["Request Response decision"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Request Response, discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Request Response to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Request Response solve better than the simpler alternative?
...
5. What would make you reject Request Response?
...
- Request Response is useful only when its core tradeoff matches the prompt.
...
Use Request Response when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Request Response when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Request Response behind a narrow interface or compatibility layer.
...
Request Response changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Request Response solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Request Response when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Request Response? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
