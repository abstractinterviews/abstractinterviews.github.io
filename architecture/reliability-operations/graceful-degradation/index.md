...
| --- | --- |
| What is it? | Graceful Degradation is a architecture fundamental topic used to make a specific engineering decision explicit. |
...
Graceful Degradation exists to help teams make structural decisions that remain understandable as the system, organization, and traffic grow.
...
```mermaid
flowchart LR
    Drivers["Business and quality drivers"] --> Decision["Graceful Degradation decision"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Graceful Degradation, discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Graceful Degradation to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Graceful Degradation solve better than the simpler alternative?
...
5. What would make you reject Graceful Degradation?
...
- Graceful Degradation is useful only when its core tradeoff matches the prompt.
...
1. Define Graceful Degradation in one sentence.
...
Use Graceful Degradation when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Graceful Degradation when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Graceful Degradation behind a narrow interface or compatibility layer.
...
Graceful Degradation changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Graceful Degradation solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Graceful Degradation when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Graceful Degradation? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
