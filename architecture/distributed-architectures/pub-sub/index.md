...
subtitle: "Pub/Sub for senior engineering and architecture interviews."
...
Pub/Sub is an architecture topic used to reason about system structure, ownership, communication, data flow, and operational behavior. In interviews, do not stop at the definition. Explain the constraints that make it attractive and the trade-offs it introduces.
...
1. Define Pub/Sub in one sentence.
...
Use Pub/Sub when its benefits directly match the system constraints. Avoid it when it adds coordination, deployment, data consistency, or operational overhead without a clear payoff.



## Imported System Design Notes

These notes were moved from the old `50-system-design-patterns/pub-sub` bucket during the taxonomy cleanup.
...
| --- | --- |
| What is it? | Pub/Sub is a architecture pattern topic used to make a specific engineering decision explicit. |
...
Pub/Sub exists to help teams make structural decisions that remain understandable as the system, organization, and traffic grow.
...
```mermaid
flowchart LR
    Drivers["Business and quality drivers"] --> Decision["Pub/Sub decision"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Pub/Sub, discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Pub/Sub to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Pub/Sub solve better than the simpler alternative?
...
5. What would make you reject Pub/Sub?
...
- Pub/Sub is useful only when its core tradeoff matches the prompt.
...
Use Pub/Sub when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Pub/Sub when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Pub/Sub behind a narrow interface or compatibility layer.
...
Pub/Sub changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Pub/Sub solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Pub/Sub when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Pub/Sub? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
