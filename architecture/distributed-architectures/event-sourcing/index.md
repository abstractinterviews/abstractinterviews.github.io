...
These notes were moved from the old `50-system-design-patterns/event-sourcing` bucket during the taxonomy cleanup.
...
| --- | --- |
| What is it? | Event Sourcing is a architecture pattern topic used to make a specific engineering decision explicit. |
...
Event Sourcing exists to help teams make structural decisions that remain understandable as the system, organization, and traffic grow.
...
```mermaid
flowchart LR
    Drivers["Business and quality drivers"] --> Decision["Event Sourcing decision"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Event Sourcing, discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Event Sourcing to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Event Sourcing solve better than the simpler alternative?
...
5. What would make you reject Event Sourcing?
...
- Event Sourcing is useful only when its core tradeoff matches the prompt.
...
Use Event Sourcing when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Event Sourcing when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Event Sourcing behind a narrow interface or compatibility layer.
...
Event Sourcing changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Event Sourcing solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Event Sourcing when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Event Sourcing? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
