...
| --- | --- |
| What is it? | Primary-Replica (Leader-Follower) is a system design concept topic used to make a specific engineering decision explicit. |
...
Primary-Replica (Leader-Follower) exists because real systems need a reusable way to manage load, coupling, correctness, latency, or change.
...
```mermaid
flowchart LR
    Problem["Interview prompt"] --> Model["Mental model for Primary-Replica (Leader-Follower)"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Primary-Replica (Leader-Follower), discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Primary-Replica (Leader-Follower) to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Primary-Replica (Leader-Follower) solve better than the simpler alternative?
...
5. What would make you reject Primary-Replica (Leader-Follower)?
...
- Primary-Replica (Leader-Follower) is useful only when its core tradeoff matches the prompt.
...
1. Define Primary-Replica (Leader-Follower) in one sentence.
...
Use Primary-Replica (Leader-Follower) when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Primary-Replica (Leader-Follower) when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Primary-Replica (Leader-Follower) behind a narrow interface or compatibility layer.
...
Primary-Replica (Leader-Follower) changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Primary-Replica (Leader-Follower) solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Primary-Replica (Leader-Follower) when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Primary-Replica (Leader-Follower)? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
