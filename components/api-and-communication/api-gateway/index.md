...
| --- | --- |
| Core idea | API Gateway helps candidates discuss component responsibility, integration contracts, bottlenecks, scaling limits, and failure behavior with clarity. |
...
- Define API Gateway in one or two crisp sentences before expanding.
...
~~~mermaid
flowchart LR
    A["Clarify the prompt"] --> B["Define API Gateway"]
...
~~~mermaid
flowchart TD
    Problem["Problem or requirement"] --> Concept["API Gateway"]
...
| --- | --- |
| Problem first | Explain what problem API Gateway solves before naming tools or patterns. |
...
| Scenario | How API Gateway Shows Up |
| --- | --- |
...
| Front | Back |
| --- | --- |
| What problem does API Gateway solve? | It helps structure a decision around requirements, constraints, trade-offs, and production impact. |
...
1. Explain API Gateway to an interviewer in 90 seconds.
