...
| --- | --- |
| What is it? | Bidirectional Streaming (WebSockets / gRPC Streaming) is a system design concept topic used to make a specific engineering decision explicit. |
...
Bidirectional Streaming (WebSockets / gRPC Streaming) exists because real systems need a reusable way to manage load, coupling, correctness, latency, or change.
...
```mermaid
flowchart LR
    Problem["Interview prompt"] --> Model["Mental model for Bidirectional Streaming (WebSockets / gRPC Streaming)"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Bidirectional Streaming (WebSockets / gRPC Streaming), discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Bidirectional Streaming (WebSockets / gRPC Streaming) to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Bidirectional Streaming (WebSockets / gRPC Streaming) solve better than the simpler alternative?
...
5. What would make you reject Bidirectional Streaming (WebSockets / gRPC Streaming)?
...
- Bidirectional Streaming (WebSockets / gRPC Streaming) is useful only when its core tradeoff matches the prompt.
...
1. Define Bidirectional Streaming (WebSockets / gRPC Streaming) in one sentence.
...
Use Bidirectional Streaming (WebSockets / gRPC Streaming) when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Bidirectional Streaming (WebSockets / gRPC Streaming) when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Bidirectional Streaming (WebSockets / gRPC Streaming) behind a narrow interface or compatibility layer.
...
Bidirectional Streaming (WebSockets / gRPC Streaming) changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Bidirectional Streaming (WebSockets / gRPC Streaming) solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Bidirectional Streaming (WebSockets / gRPC Streaming) when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Bidirectional Streaming (WebSockets / gRPC Streaming)? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
