...
| --- | --- |
| What is it? | Core Components is a system design concept topic used to make a specific engineering decision explicit. |
...
Core Components exists because real systems need a reusable way to manage load, coupling, correctness, latency, or change.
...
```mermaid
flowchart LR
    Problem["Interview prompt"] --> Model["Mental model for Core Components"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Core Components, discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Core Components to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Core Components solve better than the simpler alternative?
...
5. What would make you reject Core Components?
...
- Core Components is useful only when its core tradeoff matches the prompt.
...
1. Define Core Components in one sentence.
...
Use Core Components when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Core Components when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Core Components behind a narrow interface or compatibility layer.
...
Core Components changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Core Components solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Core Components when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Core Components? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
