...
subtitle: "Explore choices, undo state, and prune invalid paths."
...
Backtracking turns a repeated brute-force search into a maintained invariant. Instead of recomputing from scratch, you preserve just enough state to move forward safely.
...
A strong answer states brute force first, names the repeated work, then introduces Backtracking as the structure that removes it.

## Recognition Signals

### Prompt has reusable state

When you see this signal, ask whether Backtracking can preserve an invariant that avoids repeated work.
...
Maintain the Backtracking state so each decision stays valid.
...
I use Backtracking because the prompt has a reusable state. The invariant tells me which move is safe, and each step reduces the remaining search space or advances the maintained state.

## Practice

### Example 1: Choice exploration

Input: problem-specific input

Output: optimized result

Explanation: This is the standard Backtracking use case.

```python
def solve_1(input_data):
    # Apply Backtracking using the invariant above.
    return input_data
```
...
### Example 2: Backtracking Follow-up
...
| --- | --- | --- | --- |
| Backtracking | Usually O(n) to O(n log n) | Usually O(n) | Depends on state and transitions |
...
2. Backtracking Recognition
3. Template Drill

### Medium

1. Backtracking Optimization
2. Stateful Variant
3. Interview Variant
...
- Backtracking is useful only when the prompt matches its invariant.
...
- [Trees and Graphs - DFS choice](/data-structures/trees-graphs/#dfs-choice)
