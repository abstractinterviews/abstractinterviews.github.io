...
subtitle: "Store strings by shared prefixes for search and autocomplete."
...
Trie turns a repeated brute-force search into a maintained invariant. Instead of recomputing from scratch, you preserve just enough state to move forward safely.
...
A strong answer states brute force first, names the repeated work, then introduces Trie as the structure that removes it.

## Recognition Signals

### Prompt has reusable state

When you see this signal, ask whether Trie can preserve an invariant that avoids repeated work.
...
Maintain the Trie state so each decision stays valid.
...
I use Trie because the prompt has a reusable state. The invariant tells me which move is safe, and each step reduces the remaining search space or advances the maintained state.

## Practice

### Example 1: Prefix tree matching

Input: problem-specific input

Output: optimized result

Explanation: This is the standard Trie use case.

```python
def solve_1(input_data):
    # Apply Trie using the invariant above.
    return input_data
```
...
| --- | --- | --- | --- |
| Trie | Usually O(n) to O(n log n) | Usually O(n) | Depends on state and transitions |
...
1. Trie with Constraints
2. Combined Pattern
3. Adversarial Case

## Key Takeaways

- Trie is useful only when the prompt matches its invariant.
...
- [Trees and Graphs - graph mental model](/data-structures/trees-graphs/#graphs)
