...
subtitle: "Convert intervals into ordered events and scan active state."
...
Sweep Line turns a repeated brute-force search into a maintained invariant. Instead of recomputing from scratch, you preserve just enough state to move forward safely.
...
A strong answer states brute force first, names the repeated work, then introduces Sweep Line as the structure that removes it.

## Recognition Signals

### Prompt has reusable state

When you see this signal, ask whether Sweep Line can preserve an invariant that avoids repeated work.
...
Maintain the Sweep Line state so each decision stays valid.
...
I use Sweep Line because the prompt has a reusable state. The invariant tells me which move is safe, and each step reduces the remaining search space or advances the maintained state.

## Practice

### Example 1: Event ordering over coordinates

Input: problem-specific input

Output: optimized result

Explanation: This is the standard Sweep Line use case.

```python
def solve_1(input_data):
    # Apply Sweep Line using the invariant above.
    return input_data
```
...
### Example 2: Sweep Line Follow-up
...
| --- | --- | --- | --- |
| Sweep Line | Usually O(n) to O(n log n) | Usually O(n) | Depends on state and transitions |
...
1. Sweep Line with Constraints
2. Combined Pattern
3. Adversarial Case

## Key Takeaways

- Sweep Line is useful only when the prompt matches its invariant.
...
- [Selection and Complexity - ordering optimization](/data-structures/selection-complexity/#ordering-optimization)
