...
| --- | --- |
| What is it? | Common Mistakes to Avoid is a interview strategy topic used to make a specific engineering decision explicit. |
...
Common Mistakes to Avoid exists because interviewers want to see whether you can turn knowledge into a structured answer under pressure.
...
```mermaid
flowchart LR
    Problem["Interview prompt"] --> Model["Mental model for Common Mistakes to Avoid"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Common Mistakes to Avoid, discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Common Mistakes to Avoid to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Common Mistakes to Avoid solve better than the simpler alternative?
...
5. What would make you reject Common Mistakes to Avoid?
...
- Common Mistakes to Avoid is useful only when its core tradeoff matches the prompt.
...
1. Define Common Mistakes to Avoid in one sentence.
...
Use Common Mistakes to Avoid when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Common Mistakes to Avoid when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Common Mistakes to Avoid behind a narrow interface or compatibility layer.
...
Common Mistakes to Avoid changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Common Mistakes to Avoid solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Common Mistakes to Avoid when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Common Mistakes to Avoid? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
