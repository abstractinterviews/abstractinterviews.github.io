...
Definitions of important terms used in system design.

<div class="glossary-toolbar" aria-label="Glossary search">
  <input class="inline-search" type="search" placeholder="Search terms..." aria-label="Search terms">
</div>

<div class="alpha-row" aria-label="Alphabet filter">
  <span class="alpha-chip is-active">All</span>
...
| CAP Theorem | States that a distributed system can only provide two of consistency, availability, and partition tolerance at once during a partition. |
| CDN | Content Delivery Network. A distributed server network that delivers web content near the user. |
| Consistency | In distributed systems, all nodes see the same data at the same time or according to a defined consistency model. |
| Idempotent | An operation that can be applied multiple times without changing the result beyond the first application. |
| Microservices | An architectural style that structures an application as small, independent services. |
| Replication | Copying data from one node to another for redundancy, availability, or locality. |
| Sharding | Splitting data across multiple nodes by key or range to scale storage and throughput. |
...
| --- | --- |
| What is it? | Glossary is a cheat sheet topic used to make a specific engineering decision explicit. |
...
Glossary exists because interviewers want to see whether you can turn knowledge into a structured answer under pressure.
...
```mermaid
flowchart LR
    Problem["Interview prompt"] --> Model["Mental model for Glossary"]
...
> [!IMPORTANT]
> Production reality: the interview answer should mention what happens when assumptions break. For Glossary, discuss hot paths, observability, limits, backpressure, and how teams detect and recover from failures.
...
Interviewers are testing whether you can connect Glossary to constraints, tradeoffs, and failure modes. A strong answer starts simple, states assumptions, chooses the right abstraction, and then explains what would change at larger scale.

### Interview Questions

1. What problem does Glossary solve better than the simpler alternative?
...
5. What would make you reject Glossary?
...
- Glossary is useful only when its core tradeoff matches the prompt.
...
1. Define Glossary in one sentence.
...
Use Glossary when it directly improves the dominant constraint: lookup speed, coupling, scalability, reliability, delivery speed, or reasoning clarity.

### Why Not Use It

Avoid Glossary when the simpler approach already meets the requirements, when operational overhead exceeds the benefit, or when the team cannot own the added complexity.

### Migration Strategy

1. Start with the simplest working design and capture baseline metrics.
2. Introduce Glossary behind a narrow interface or compatibility layer.
...
Glossary changes how teams communicate. It may require clearer ownership, better contracts, shared vocabulary, and explicit review of cross-team dependencies.
...
- Glossary solves a specific pressure; name that pressure first.
...
**Common interview answer:** "I would use Glossary when the constraints make its tradeoff worthwhile. I would start with the simplest version, validate the bottleneck, then add this structure or pattern where it improves the hot path while keeping failure modes observable."
...
1. **Q:** What is the main purpose of Glossary? **A:** To solve a specific constraint or reasoning problem more clearly than a naive approach.
